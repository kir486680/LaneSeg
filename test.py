...
class WrappedDeeplabv3Resnet101(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = UNet_ConvLSTM(config.img_channel, config.class_num).eval()
...
def output_result(model, test_loader, device):
    ...
    with torch.no_grad():
        for sample_batched in test_loader:
            ...
            data = data.squeeze(0)
            print("The input shape",data.shape)
...
if __name__ == '__main__':
    ...
    pretrained_dict = torch.load(config.pretrained_path, map_location=torch.device('cpu'))
